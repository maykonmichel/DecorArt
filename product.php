    ...
    <body>
        ...
        <div class="main">
            ...
            <div class="product-details" id="<?php echo $product['id'] ?>">
            	...
            	<h4>Nome</h4>
                <h1><?php echo $product['tipo'] ?></h1>
            	<h4>Tipo</h4>
                <h1><?php echo $product['tema'] ?></h1>
            	<h4>Tema</h4>
                <h1><?php echo $product['altura'] ?>cm</h1>
            	<h4>Altura</h4>
                <h1><?php echo $product['largura'] ?>cm</h1>
            	<h4>Largura</h4>
                <h1><?php echo $product['preco'] ?></h1>
            	<h4>Preço</h4>
            </div>
            <form action="add-to-cart.php" method="get" id="add-to-cart">
                <?php
					$cart = json_decode($_COOKIE['cart'], true);
					if($product['estoque']-$cart[$id]) {
						echo '
							<span class="add-to-cart right-align" onClick="document.getElementById(\'add-to-cart\').submit();">
                    			Adicionar ao carrinho
                			</span>
							<input type="number"
								name="amount"
								id="input-number-cart"
								min="1"
								max="'.($product['estoque']-$cart[$id]).'"
								step="1"
								value="1" 
						...
						echo '
							<span id="sold-out">Produto esgotado</span>
						';
					}
				?>
                
                <input type="text" name="id" value="<?php echo $product['id'] ?>" style="display:none">
            </form>
